# Stage 7 · Testing · Part 2 — Integration Tests
...
class TestProductCreate:
    ...
    async def test_create_product_calculates_true_cost_with_loss(self, auth_client):
        resp = await auth_client.post("/api/products/", json={
            "name": "Joha Rice 2", "category": "rice", "unit": "kg",
    ...
    async def test_float_price_rejected(self, auth_client):
        resp = await auth_client.post("/api/products/", json={
            "name": "Bad Product", "category": "rice", "unit": "kg",
    ...
    async def test_gross_margin_calculated_in_response(self, auth_client):
        resp = await auth_client.post("/api/products/", json={
            "name": "Margin Test", "category": "petha", "unit": "pc",
            "sell_price": "70", "farm_cost": "20", "labor_cost": "5",
        })
        assert resp.status_code == 201
        data = resp.json()
...
class TestProductUpdate:
    async def test_update_price_recalculates_margin(self, auth_client, joha_product):
        ...
        resp = await auth_client.patch(f"/api/products/{product.id}", json={
            "sell_price": "120",
        })
        assert resp.status_code == 200
...
class TestMonthlyStock:
    async def test_set_opening_stock(self, auth_client, joha_product):
        product,_ = joha_product
        resp = await auth_client.post("/api/inventory/monthly-stock", json={
            "product_id": str(product.id),
            "month":      "2025-05",
            "stock_type": "opening",
            "qty":        "100",
