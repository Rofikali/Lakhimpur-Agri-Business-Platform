...
def order_payload(
    product_id: str, qty: str = "3", payment_mode: str = "cash", channel: str = "offline"
) -> dict:
    return {
...
class TestOrderCreate:
    ...
    async def test_offline_order_decrements_stock(self, auth_client, joha_product, mock_wati):
        ...
        resp = await auth_client.post("/api/orders/", json=order_payload(str(product.id), qty="3"))
        assert resp.status_code == 201
    ...
    async def test_online_order_stays_pending(self, auth_client, joha_product, mock_razorpay):
        ...
        resp = await auth_client.post(
            "/api/orders/",
            json=order_payload(str(product.id), payment_mode="razorpay", channel="online"),
        )
        assert resp.status_code == 201
        data = resp.json()
    ...
    async def test_oversell_rejected(self, auth_client, joha_product):
        ...
        resp = await auth_client.post(
            ...
            json=order_payload(
                str(product.id),
                qty="100",  # ← more than 50 available
            ),
        )
...
class TestOrderStatusUpdate:
    ...
    async def test_cancel_restores_stock(self, auth_client, joha_product, mock_wati):
        ...
        order_resp = await auth_client.post(
            "/api/orders/", json=order_payload(str(product.id), qty="5")
        )
        order_id = order_resp.json()["id"]
        ...
        # Cancel the order
        cancel = await auth_client.patch(
            f"/api/orders/{order_id}/status",
            json={
                "status": "cancelled",
...
class TestWebhook:
    ...
    async def test_mark_paid_offline_order(self, auth_client, joha_product, mock_wati):
        ...
        resp = await auth_client.patch(
            f"/api/payments/{order_id}/mark-paid",
            json={
                "payment_mode": "cash",
            },
        )
...
class TestFarmSeasonLifecycle:
    ...
    async def test_add_cultivation_input(self, auth_client):
        season = await auth_client.post(
            "/api/farm/seasons",
            json={"variety": "bora_saul", "area_bigha": "2", "start_date": "2025-06-01"},
        )
        season_id = season.json()["id"]

        resp = await auth_client.post(
            f"/api/farm/seasons/{season_id}/inputs",
            json={
                "input_type": "seed",
    ...
    async def test_record_harvest(self, auth_client):
        season = await auth_client.post(
            "/api/farm/seasons",
            json={"variety": "kali_jeera", "area_bigha": "1", "start_date": "2025-06-01"},
        )
        season_id = season.json()["id"]

        resp = await auth_client.post(
            f"/api/farm/seasons/{season_id}/harvest",
            json={
                "dhan_qty_kg": "800",
    ...
    async def test_milling_calculates_yield_and_cost(self, auth_client, joha_product):
        season = await auth_client.post(
            "/api/farm/seasons",
            json={"variety": "joha", "area_bigha": "3", "start_date": "2025-06-01"},
        )
        season_id = season.json()["id"]
        ...
        # Record milling
        resp = await auth_client.post(
            f"/api/farm/seasons/{season_id}/milling",
            json={
                "dhan_sent_kg": "1200",
    ...
    async def test_cannot_mill_before_harvest(self, auth_client):
        season = await auth_client.post(
            "/api/farm/seasons",
            json={"variety": "joha", "area_bigha": "2", "start_date": "2025-06-01"},
        )
        season_id = season.json()["id"]
        ...
        resp = await auth_client.post(
            ...
            json={
                "dhan_sent_kg": "1000",
                "chawl_received_kg": "650",
                "milling_charges": "1500",
                "milling_date": "2025-11-01",
            },
        )
        assert resp.status_code == 422
    ...
    async def test_fail_season(self, auth_client):
        ...
        resp = await auth_client.patch(
            f"/api/farm/seasons/{season_id}/fail",
            json={
...
class TestPethaLifecycle:
    async def test_create_batch_and_record_outcome(self, auth_client, narikal_product):
        ...
        # Record outcome
        outcome = await auth_client.patch(
            f"/api/petha/batches/{batch_id}/outcome",
            json={
                "good_pieces": 27,
                "rejected_pieces": 3,
            },
                ...
                "costs": [
                    {
                        ...
                        "description": "guri",
                        "qty": "1",
                        "unit_cost": 50.5,  # ← float — must fail
                        "total_amount": "50.5",
                    },
                ],
            },
    ...
    async def test_expiring_soon_endpoint(self, auth_client, narikal_product):
        ...
        await auth_client.patch(
            f"/api/petha/batches/{batch_id}/outcome", json={"good_pieces": 10, "rejected_pieces": 0}
        )
