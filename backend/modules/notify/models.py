...
class Notification(Base):
    ...
    recipient_type: Mapped[str] = mapped_column(SAEnum("owner", "customer", name="notification_recipient_type"))
    channel: Mapped[str] = mapped_column(SAEnum("whatsapp", "sms", name="notification_channel"))
    template_name: Mapped[str] = mapped_column(String(100))
    message_body: Mapped[str | None] = mapped_column(Text, nullable=True)
    status: Mapped[str] = mapped_column(SAEnum("pending", "sent", "failed", name="notification_status"), default="pending")
